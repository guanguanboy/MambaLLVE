...
from .rain_mamba_basic_restorer import RainMambaBasicRestorer
...
class RainMambaBRVE(RainMambaBasicRestorer):
        ...
        with torch.no_grad():
            ...
            for i in range(0, t, self.clip_length):
                ...
                if self.test_cfg is not None and self.test_cfg.get('pad', None):
                    ...
                    output[:, i:i+self.clip_length] = self.pad2D_test(lq[:, i:i+self.clip_length], pad, hilbert_curve_large_scale, hilbert_curve_small_scale)
                # output[:, i:i+self.clip_length] = self.generator(lq[:, i:i+self.clip_length])[:, :, :, :h, :w]
                    ...
                    output[:, i:i+self.clip_length] = self.tile2D_test(lq[:, i:i+self.clip_length], tile, tile_overlap, hilbert_curve_large_scale, hilbert_curve_small_scale)
                else:
                    output[:, i:i+self.clip_length] = self.generator(lq[:, i:i+self.clip_length], hilbert_curve_large_scale, hilbert_curve_small_scale)
            output = output[:, :t, :, :, :]
            print(output.shape)
    ...
    def pad2D_test(self, lq, pad, hilbert_curve_large_scale, hilbert_curve_small_scale):
        ...
        print(img.size())
        output = self.generator(img, hilbert_curve_large_scale, hilbert_curve_small_scale)
        print(output.shape)
        print(output[:, :, :, 0:h, 0:w].shape)
        return output[:, :, :, 0:h, 0:w]
    
    def tile2D_test(self, lq, tile, tile_overlap, hilbert_curve_large_scale, hilbert_curve_small_scale):
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                ...
                out_patch = self.generator(in_patch, hilbert_curve_large_scale, hilbert_curve_small_scale)
    ...
    def forward_dummy(self, img):
        ...
        with torch.no_grad():
            ...
            for i in range(0, t, self.clip_length):
                print(i, i+self.clip_length)
                out = self.generator(img[:, i:i+self.clip_length], hilbert_curve_large_scale, hilbert_curve_small_scale)
        return out
...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=4, out_channels=6, kernel_size=3, padding=1)  # 从4通道到6通道
        self.conv2 = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=3, padding=1)  # 从6通道到3通道
...
class ConvLast(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=6, kernel_size=3, padding=1)  # 从3通道到6通道
        self.conv2 = nn.Conv2d(in_channels=6, out_channels=4, kernel_size=3, padding=1)  # 从6通道到4通道
        self.relu = nn.ReLU()  # 激活函数
...
class MambaLLIE(nn.Module):
    def __init__(self, nf=32,
        ...
        self.conv_last = ConvLast()

    def forward(self, lqs):

        n, t, c, h, w = lqs.size()

        x_in = self.domain_conv(lqs.view(-1, c, h, w))

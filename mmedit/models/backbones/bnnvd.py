...
class BNNVD(nn.Module):
    ...
    def forward(self, lqs):
        ...
        print('lgs_view_size=', lqs.view(-1, c, h, w).size())
        feat_in = self.feat_extract(lqs.view(-1, c, h, w))
        print('feat_in_size=', feat_in.size())

        stage0_feat = self.stage0(feat_in)
        print('stage0_feat_size=', stage0_feat.size())
        ...
        print('fusion_in size =', fusion_in.size())
        stage2_feats = self.stage2(self.fusion(fusion_in))
        print('stage2_feats size =', stage2_feats.size())
        convout_feats = self.conv_out(stage2_feats)
        print('convout_feats size =', convout_feats.size())
        out = lqs[:, :, :4, :, :] + convout_feats.view(n, t, -1, h, w)
